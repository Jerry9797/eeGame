...
import (
	"context"
	"crypto/ecdsa"
	contract2 "eeGame/internal/logic/trade/contract_metadata"
...
type Ee struct {
	...
	D711Engine            *contract2.D711Engine
	D711EngineAddress     common.Address
	D711StableCoin        *contract2.D711StableCoin
...
func NewEe(ctx context.Context) (*Ee, error) {
	...
	d711Engine, err := contract2.NewD711Engine(d711EngineAddress, client)
	if err != nil {
		return nil, fmt.Errorf("failed to load DSC Engine contract_metadata: %v", err)
	}
	d711, err := contract2.NewD711StableCoin(d711StableCoinAddress, client)
	if err != nil {
		return nil, fmt.Errorf("failed to load DSC Engine contract_metadata: %v", err)
	}
...
func (t *Ee) depositCollateralAndMintD711(nonce uint64, gasFeeCap *big.Int, gasTipCap *big.Int, err error, amountCollateral *big.Int, amountDscToMint *big.Int) error {
	...
	err = t.getGasLimit(auth, t.D711EngineAddress, contract2.D711EngineMetaData.ABI, "depositCollateralAndMintD711",
...
func (t *Ee) approveWETH(nonce uint64, gasFeeCap, gasTipCap *big.Int, amountCollateral *big.Int) error {
	weth9, err2 := contract2.NewWETH9(t.wethAddress, t.client)
	if err2 != nil {
		panic(err2)
	}
	...
	//	}
	auth := t.createTransactor(nonce, gasFeeCap, gasTipCap)
	err := t.getGasLimit(auth, t.wethAddress, contract2.WETH9MetaData.ABI, "approve", t.D711EngineAddress, amountCollateral)
	if err != nil {
		return err
	}
...
func (t *Ee) GetBalanceERC20(address common.Address) (*big.Int, error) {
	coin, err := contract2.NewD711StableCoin(t.D711StableCoinAddress, t.client)
	if err != nil {
		return nil, err
	}
	opts := t.creatCallOpts()
...
func (t *Ee) redeemCollateralForDsc(nonce uint64, gasPrice *big.Int, gasTipPrice *big.Int, d711Amount *big.Int) error {
	...
	err = t.getGasLimit(auth, t.D711EngineAddress, contract2.D711EngineMetaData.ABI, "redeemCollateralForD711",
...
func (t *Ee) approveD711(nonce uint64, gasFeeCap, gasTipPrice *big.Int, d711Amount *big.Int) error {
	d711Coin, err := contract2.NewD711StableCoin(t.D711StableCoinAddress, t.client)
	if err != nil {
		logrus.Errorf("NewDSC error:%v", err)
	}
	auth := t.createTransactor(nonce, gasFeeCap, gasTipPrice)
	err = t.getGasLimit(auth, t.D711StableCoinAddress, contract2.D711StableCoinMetaData.ABI, "approve", t.D711EngineAddress, d711Amount)
	if err != nil {
		return err
	}

...
const (
	HeaderLen     = 3 // 1byte package type 3byte body len
	MaxPacketSize = 1 << 24
)

type Packet struct {
	Type      PackageType `json:"type"`
	RouteCode uint16      `json:"routeCode"`
	Body      Sys         `json:"body"`
}
...
func Decode(payload []byte) (*Packet, error) {
	...
	if p.Type == Data {
		...
		//}
		//p.Body = m
		_, ok := codes[p.RouteCode]
		if !ok {
			return nil, fmt.Errorf("路由未注册")
		}
...
func SetDictionary(dict map[string]uint16) {
	...
	for route, code := range dict {
		...
		// duplication check
		if _, ok := routes[r]; ok {
			logrus.Errorf("duplicated route(route: %s, code: %d)", r, code)
			return
		}
...
func (p *Packet) GetRouteByCode(code uint16) string {
	route, ok := codes[code]
	if ok {
		return route
	}
